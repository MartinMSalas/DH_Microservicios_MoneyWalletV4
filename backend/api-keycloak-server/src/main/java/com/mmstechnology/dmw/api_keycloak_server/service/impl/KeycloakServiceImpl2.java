 ...
 */
@Slf4j
public class KeycloakServiceImpl2 implements IKeycloakService {
    ...
    private void assignRolesToUser(String userId, Set<String> roles) {
        ...
        if (roles == null || roles.isEmpty()) {
            roleRepresentations = List.of(realmResource.roles().get(DEFAULT_ROLE).toRepresentation());
        } else {
            roleRepresentations = roles.stream()
                    .map(roleName -> Optional.ofNullable(realmResource.roles().get(roleName))
                            .map(RoleRepresentation::toRepresentation)
                            .orElse(null))
